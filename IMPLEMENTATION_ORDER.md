...
Rev 40, 2026-08-23. **DCH-75 shipped** (filed and built same-day: photo carousel on Saved
Listings cards). The DCH-52 carousel came out of `SellerFeed.tsx` into the shared
`CarouselThumbnail` + pure `lib/carousel.ts` (`stepIndex`, `visibleImages` — the
empty-detail fallback rule is what keeps a photo-less detail from replacing the one known
image with an empty frame). The Saved Listings card gained a "Photos" toggle under the
thumbnail with **card-local** fetch state — nothing new crosses the DCH-58 memo boundary,
and the fetch is `feed_item_detail`, which serves watched rows from their own `raw_json`
...
**version + short commit** in the title (`Diecast Hunter 26.8.17 (cd658ea)`; commit is
what tells two same-day CI builds apart, `-dirty` marks unclean local builds), and
**derived at build time** in `build.rs` — build date → CalVer, `git rev-parse` → commit;
nothing hand-edited. CI stamps the same CalVer into the installers via
`tauri build --config`; the static version in `tauri.conf.json` is only a fallback for
local builds. Also surfaced as `app:` in the Dashboard diagnostics card and in
`app_status`, so a bug report can quote it (the DCH-24 tie-in). Next: DCH-68.
...
| DCH-75 | Photo carousel on Saved Listings cards: DCH-52's carousel extracted to the shared `CarouselThumbnail` (+ tested `lib/carousel.ts`), mounted behind a "Photos" toggle with card-local fetch state; served from `raw_json` (zero Browse quota); withheld on archived rows (URLs rot). |
